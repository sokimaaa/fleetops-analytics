package com.fleetops.analytics.domain.validation
...
class YellowTripSourceValidatorSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("invalidRecords returns only rows with non-empty validation_errors") {
    val session = spark
    import session.implicits._
    ...
    val invalidIds = YellowTripSourceValidator
